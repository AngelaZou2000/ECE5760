  ...
  for (i = 0; i < 640; i++)
  {
    for (j = 0; j < 480; j += 1)
    {
      ...
      if (count >= max_count)
        ...
        VGA_PIXEL(i, j, 0xe0); // e0
      else if (count >= (max_count >> 2))
        VGA_PIXEL(i, j, 0xe0); // e0
      else if (count >= (max_count >> 3))
        VGA_PIXEL(i, j, 0xc0); // e0
      else if (count >= (max_count >> 4))
        VGA_PIXEL(i, j, 0xc0); // c0
      else if (count >= (max_count >> 5))
        VGA_PIXEL(i, j, 0xa0); // a0
      else if (count >= (max_count >> 6))
        VGA_PIXEL(i, j, 0x80); // 80
      else if (count >= (max_count >> 7))
        VGA_PIXEL(i, j, 0x24); // 40
      else
        VGA_PIXEL(i, j, 0x24); // dark gray
    }
  }
